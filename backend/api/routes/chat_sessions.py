...
class ChatSessionsPutResponseDTO(BaseModel):
    ok: bool = True
    stateRevision: int
...
@router.put("/state", response_model=ChatSessionsPutResponseDTO)
async def put_chat_sessions_state(
    body: ChatSessionsPutDTO,
) -> ChatSessionsPutResponseDTO:
    ...
    try:
        ...
        try:
            ...
            raise ApiException(
                ...
                message="chat session state was modified by another client; reload before saving",
                status_code=409,
                detail={"stateRevision": exc.current_revision},
            ) from exc
        return ChatSessionsPutResponseDTO(ok=True, stateRevision=revision)
    finally:
        sqlite.close()
