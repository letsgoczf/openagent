...
class UIChatStateRevisionConflict(Exception):
    """Raised when a UI chat state write is based on an old snapshot."""
...
class SQLiteStore:
    ...
    def _get_ui_chat_revision(self) -> int:
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
            ("chat_sessions_revision",),
        ).fetchone()
        try:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        """返回 active_session_id、sessions 和用于并发保护的 state_revision。"""
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
        ...
        return active, sessions, self._get_ui_chat_revision()
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表；只有基于最新 revision 的快照才允许写入。"""
        self._conn.execute("BEGIN IMMEDIATE")
        try:
            current_revision = self._get_ui_chat_revision()
            if base_revision != current_revision:
                raise UIChatStateRevisionConflict(current_revision)
            ...
            self._conn.execute(
                ...
                ON CONFLICT(key) DO UPDATE SET value = excluded.value
                """,
                ("chat_sessions_revision", str(next_revision)),
            )
            self._conn.commit()
