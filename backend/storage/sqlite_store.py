...
class UIChatStateConflictError(Exception):
    """Raised when a UI chat state save is based on a stale revision."""

    def __init__(self, *, expected_revision: int, actual_revision: int) -> None:
        super().__init__(
            f"stale UI chat state revision: expected {expected_revision}, got {actual_revision}"
        )
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        """返回 active_session_id、sessions 列表和全局 state revision。"""
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表；base_revision 不匹配时拒绝旧快照覆盖。"""
