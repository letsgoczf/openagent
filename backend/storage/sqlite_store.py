...
UI_CHAT_REVISION_KEY = "ui_chat_state_revision"
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        ...
        rev_row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
            (UI_CHAT_REVISION_KEY,),
        ).fetchone()
        try:
            revision = int((rev_row["value"] if rev_row else "") or 0)
        except ValueError:
            revision = 0
        cur = self._conn.execute(
            """
        ...
        if revision == 0 and sessions:
            ...
            self._conn.execute(
                ...
                """,
                (UI_CHAT_REVISION_KEY, str(revision)),
            )
            self._conn.commit()
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        ``expected_revision`` 用于前端多标签页的乐观锁；不匹配时不写入。
        """
        ...
        try:
            ...
            try:
                current_revision = int((rev_row["value"] if rev_row else "") or 0)
            except ValueError:
                current_revision = 0
            if current_revision == 0:
                has_sessions = self._conn.execute(
                    "SELECT 1 FROM ui_chat_session LIMIT 1"
                ).fetchone()
                if has_sessions:
                    current_revision = 1
            if expected_revision is None and current_revision > 0:
                raise ValueError("missing base revision for existing ui chat state")
            if expected_revision is not None and expected_revision != current_revision:
                raise ValueError(
                    f"stale ui chat state revision: expected {expected_revision}, got {current_revision}"
                )
            ...
            self._conn.execute(
                ...
                ON CONFLICT(key) DO UPDATE SET value = excluded.value
                """,
                (UI_CHAT_REVISION_KEY, str(next_revision)),
            )
            self._conn.commit()
