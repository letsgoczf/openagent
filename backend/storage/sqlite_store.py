...
class UiChatStateConflict(RuntimeError):
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        """返回 (active_session_id, sessions, revision)，结构与前端 ChatSessionPersisted 一致。"""
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表（单用户；事务），返回新的 revision。"""
        ...
        try:
            ...
            self._conn.execute(
                ...
                ON CONFLICT(key) DO UPDATE SET value = excluded.value
                """,
                ("chat_state_revision", str(new_revision)),
            )
            self._conn.commit()
            return new_revision
        except Exception:
