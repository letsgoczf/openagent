...
class ChatSessionsStateConflictError(Exception):
    """Raised when a UI chat state snapshot is based on a stale revision."""

    def __init__(self, *, current_revision: int, base_revision: int) -> None:
        super().__init__(
            f"chat sessions state revision conflict: current={current_revision}, base={base_revision}"
        )
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[int, str | None, list[dict[str, Any]]]:
        """返回 (revision, active_session_id 或 None, sessions 列表)。"""
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
            ("active_chat_session_id",),
        ).fetchone()
        revision = self._ui_chat_state_revision()
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表（单用户；带乐观锁事务）。"""
        self._conn.execute("BEGIN IMMEDIATE")
        try:
            current_revision = self._ui_chat_state_revision()
            if current_revision != base_revision:
                raise ChatSessionsStateConflictError(
