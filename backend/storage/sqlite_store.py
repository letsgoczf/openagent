...
class UIChatStateConflictError(Exception):
    """Raised when a UI chat snapshot is based on a stale state revision."""

    def __init__(self, *, current_revision: int, base_revision: int) -> None:
        super().__init__(
            f"stale UI chat state revision: base={base_revision}, current={current_revision}"
        )
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        """返回 (active_session_id, sessions, state_revision)，sessions 与前端结构一致。"""
        row = self._conn.execute(
            "SELECT value FROM ui_preferences WHERE key = ?",
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表（单用户；事务），并用 revision 防止旧快照覆盖新数据。"""
