...
class UIChatStateConflictError(Exception):
    """Raised when a full-state UI chat save is based on a stale revision."""
...
class SQLiteStore:
    ...
    def get_ui_chat_state(self) -> tuple[str | None, list[dict[str, Any]], int]:
        """返回 (active_session_id, sessions, revision)。"""
        row = self._conn.execute(
    ...
    def put_ui_chat_state(
        ...
        *,
        ...
        """全量替换 UI 会话表；expected_revision 提供 CAS 保护。"""
        ...
        try:
            ...
            if expected_revision is not None and expected_revision != current_revision:
                raise UIChatStateConflictError(
                    f"stale chat session state revision: expected "
