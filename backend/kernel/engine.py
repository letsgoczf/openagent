...
def _should_persist_memory(result: ChatRunResult, assistant_body: str) -> bool:
    """Only durable, complete assistant turns should become long-term memory."""
    if not assistant_body.strip():
        return False
        ...
        *,
        ...
        try:
            ...
            if self.settings.memory.enabled:
                ...
                if _should_persist_memory(result, body):
                    trace.emit(
                        "memory_write",
                        {
                            "session_id": sid,
