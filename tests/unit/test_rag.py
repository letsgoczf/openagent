...
def test_retrieval_excludes_unfinished_document_versions(tmp_path) -> None:
    db = tmp_path / "statuses.db"
    store = SQLiteStore(db)
    ...
    for status in ["ready", "completed", "processing", "failed"]:
        ...
        seeded[status] = chunk_id
        store.insert_document(doc_id, f"/tmp/{status}.txt", f"{status}.txt", "text/plain")
    ...
    returned_ids = {entry.chunk_id for entry in result.evidence_entries}

    assert seeded["ready"] in returned_ids
    assert seeded["completed"] in returned_ids
    assert seeded["processing"] not in returned_ids
    assert seeded["failed"] not in returned_ids
