...
def test_retrieval_defaults_to_completed_or_ready_versions(tmp_path) -> None:
    ...
    assert {entry.version_id for entry in result.evidence_entries} == expected_versions
    assert result.retrieval_state["version_scope_source"] == "retrievable_status"
    assert result.retrieval_state["version_scope_count"] == 2
    store.close()
    qd.close()


def test_empty_explicit_version_scope_returns_no_retrieval(tmp_path) -> None:
    db = tmp_path / "empty_scope.db"
    store = SQLiteStore(db)
    doc_id, ver_id = str(uuid.uuid4()), str(uuid.uuid4())
    cid = str(uuid.uuid4())
    store.insert_document(doc_id, "/tmp/x.pdf", "x.pdf", "pdf")
    store.insert_document_version(ver_id, doc_id, "h", "ev1", "tok", "ready")
    store.insert_chunk(cid, ver_id, "text", 0, "needle", {"page_number": 1}, page_number=1)
    qd = QdrantStore("empty_scope", vector_size=4, location=":memory:")
    qd.ensure_collection()
    vec = [1.0, 0.0, 0.0, 0.0]
    qd.upsert_embedding(
        vec,
    ...
    svc = RetrievalService(store, qd, TokenizerService(model_id="gpt-4"), settings=_settings())
    result = svc.retrieve("needle", vec, version_scope=[])

    assert result.evidence_entries == []
    assert result.citations == []
    assert result.retrieval_state["version_scope_count"] == 0
    store.close()
    qd.close()
